...
int main() {
    ...
    // Allocate buffer
    buffer = (unsigned char *)malloc(1024 * 1024); // 1 MB buffer
    if (!buffer) {
    ...
    while (1) {
        ...
        printf("Enter command (edit <offset> <value>, undo, save, quit): ");
        scanf("%s", command);
        ...
        if (strcmp(command, "edit") == 0) {
            ...
            if (offset >= 0 && offset < size) {
                pushUndo(offset, buffer[offset]); // Store old value for undo
...
void loadFile(const char *filename, unsigned char *buffer, long *size) {
    ...
    *size = fread(buffer, 1, 1024 * 1024, file); // Read up to 1 MB
    fclose(file);
}
...
// Display hexadecimal and ASCII representation of the buffer
void displayHex(const unsigned char *buffer, long size, long offset) {
...
// Pop an undo action from the stack
unsigned char popUndo(long *offset) {
    if (undoCount > 0) {
